...
Window {
    ...
    Backend {
        id: backend
        onSmbConnected: {
            ti.append(addMsg("Новое подключение"));
        }
        onSmbDisconnected: {
            ti.append(addMsg("Отключение пользователя"));
        }
        onNewMessage: {
            ti.append(addMsg("Новое сообщение: " + msg));
        }
    }
    ColumnLayout
    {
